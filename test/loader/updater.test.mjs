...
const { getFolderName, getUpdaterVersion, clearUpdaterCache } = require("../../dist/loader/updater.js");
const { S } = require("../../dist/loader/state.js");
const { REPOS_DIR } = require("../../dist/loader/env.js");
...
describe("updater: the resolved plugin manager", () => {
  it("imports the deployed bundle of whichever plugin declares plugin-management", async () => {
    const { preloadUpdater, getUpdater, getUpdaterPath, getUpdaterVersion, clearUpdaterCache, resolvedManager, managerBootstrapCommand } = require("../../dist/loader/updater.js");
    const { PLUGINS_DIR, REPOS_DIR } = require("../../dist/loader/env.js");
  ...
  it("answers null and names no plugin when nothing in the home declares the capability", async () => {
    const { preloadUpdater, getUpdater, clearUpdaterCache, resolvedManager, managerBootstrapCommand } = require("../../dist/loader/updater.js");
    const { PLUGINS_DIR, REPOS_DIR, CACHE_DIR } = require("../../dist/loader/env.js");
  ...
  it("stays on disk with no argument, and queries only the injected seam", async () => {
    const { preloadUpdater, clearUpdaterCache, resolvedManager } = require("../../dist/loader/updater.js");
    const { CACHE_DIR } = require("../../dist/loader/env.js");
    rmSync(join(CACHE_DIR, "plugin-manager.json"), { force: true });
...
describe("updater: setupPlugin", () => {
  it("runs the resolved entry even when no package directory is known", async () => {
    const { setupPlugin } = require("../../dist/loader/updater.js");
